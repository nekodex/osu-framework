...
## `fetch_and_build.sh`
This script downloads ffmpeg 5.0 from ffmpeg.org and compiles it. You should run this twice - once for x86_64 and then arm64 (on relevant hardware as required).
...
## Cleanup
Files are left around for debugging purposes, manually delete the `build-$arch` and `ffmpeg-5.0` folders to cleanup.
